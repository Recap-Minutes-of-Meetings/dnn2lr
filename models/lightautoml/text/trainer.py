...
class Trainer:
    ...
    def fit(self, dataloaders: Dict[str, DataLoader]) -> np.ndarray:
        ...
        if self.grad_save_path is not None:
            for stage in ['train', 'val']:
                grads = self.get_grads(dataloaders[stage], stage)
                joblib.dump(grads, self.grad_save_path + "/" + stage + "_grads.pkl")
    ...
    def train(self, dataloaders: Dict[str, DataLoader]) -> List[float]:
        ...
        for sample in loader:
            data = {
        i: (sample[i].long()[:2, ...].to(self.device) if _dtypes_mapping[i] == "long" else sample[i][:2, ...].to(self.device))
        ...
        # for stage in ['val', 'test']:
            ...
            for _ in range(1):
                for sample in loader:
                    ...
                    for j in range(data['cat'].shape[0]):
                        self.model.zero_grad()
                        _data = {
                            i: (data[i][j:j+1] if _dtypes_mapping[i] == "long" else data[i][j:j+1])
                            for i in data.keys()
                        }
                        
                        loss = self.model(_data).mean()
                        loss.backward()
                        grads = torch.hstack([x.weight.grad.data[cat_id] for cat_id, x in zip(_data['cat'].flatten(), self.model.cat_embedder.emb_layers)])
                        res_grads[stage].append(grads.detach().cpu())
                    
                    res_data[stage].append(data['cat'].detach().cpu())
            
            res_grads[stage] = torch.stack(res_grads[stage])
            res_data[stage] = torch.vstack(res_data[stage])
        
        return {"emb": [x.weight.data.cpu().numpy() for x in self.model.cat_embedder.emb_layers],
                "grads": res_grads[stage].numpy(),
                "vals": res_data[stage].numpy()}
    ...
    def predict(self, dataloader: DataLoader, stage: str) -> np.ndarray:
        ...
        if self.grad_save_path is not None:
            grads = self.get_grads(dataloader, stage)
            joblib.dump(grads, self.grad_save_path + "/test_grads.pkl")
